...
router.get("/", anyAuth, async (_req: Request, res: Response): Promise<void> => {
  const { data, error } = await supabase.from("device_config").select("*").eq("id", 1).single();
  if (error) {
...
router.put("/", userAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (rain_active !== undefined) {
    if (!["LOW", "HIGH"].includes(rain_active)) {
      res.status(400).json({ error: 'rain_active must be "LOW" or "HIGH"' });
      return;
    }
  ...
  if (led_mode !== undefined) {
    if (!["solid", "blink"].includes(led_mode)) {
      res.status(400).json({ error: 'led_mode must be "solid" or "blink"' });
      return;
    }
  ...
  if (led_blink_ms !== undefined) {
    ...
    if (ms < 100 || ms > 5000) {
      res.status(400).json({ error: "led_blink_ms must be 100–5000" });
      return;
    }
